...
pub fn write_variable(
    ...
    println!("debuglog:: at write_variable, flitered_value={0}",flitered_value);
    
	{
		mask = (make_all_ones(target_l_width) << (8 - target_l_width)) as u8;
		value_byte = ((flitered_value & make_all_ones(target_l_width)) << (8 - target_l_width)) as u8;
        println!("debuglog:: Processing leftwidth, tar_l_wid={0},tar_l={1},mask={2},valuebyte={3}",target_l_width,target_l,mask,value_byte);
	...
	for i in target_l+1..target_r {
        ...
        println!("debuglog:: Processing midwidth, i={0},mask={1},valuebyte={2}",i,mask,value_byte);
    ...
    if target_l<target_r
	{
		...
		value_byte = (get_range_bits(flitered_value, bit_processed, width) & make_all_ones(target_r_width)) as u8;
...
fn get_range_bits(
    ...
    println!("debuglog:: get_range_bits: value={0},left={1},rig={2}",value,left_side,right_side);
