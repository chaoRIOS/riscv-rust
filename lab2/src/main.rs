...
fn run_elf(input_path: &str, trace_path: &str, trace_memory_access: bool, mem_dump: &str) -> std::io::Result<()> {
				...
				Some(input_path) => {
					match _args.opt_str("t") {
						...
						Some(trace_path) => {
							...
							run_elf(input_path.as_str(), trace_path.as_str(), true, mem_dump.as_str())?
						}
